...
var NEVER = INVALID;
// src/schemas.ts
var WorkflowNodeSchema = exports_external.object({
  id: exports_external.string(),
  type: exports_external.string()
}).passthrough();
var StandardEdgeSchema = exports_external.object({
  from: exports_external.string(),
  to: exports_external.string(),
  type: exports_external.literal("standard")
});
var ConditionalEdgeSchema = exports_external.object({
  from: exports_external.string(),
  type: exports_external.literal("conditional"),
  ...
  routes: exports_external.object({
    true: exports_external.string(),
    false: exports_external.string()
  })
});
var EdgeSchema = exports_external.discriminatedUnion("type", [
  StandardEdgeSchema,
  ConditionalEdgeSchema
]);
var WorkflowSchema = exports_external.object({
  version: exports_external.literal("1.0"),
  graph_id: exports_external.string(),
  entry_point: exports_external.string(),
  nodes: exports_external.array(WorkflowNodeSchema),
  edges: exports_external.array(EdgeSchema)
});
function parse(raw) {
...
function resolveNextNode(currentNodeId, edges, state) {
...
// src/execute.ts
async function _execute(workflow, initialState, options, emit) {
  const {
    handlers,
    registry = {},
    maxSteps = 1000,
  ...
  let currentNodeId = workflow.entry_point;
  ...
  while (currentNodeId !== undefined) {
    ...
    try {
      if (node.type === "sub_workflow") {
        ...
        currentState = await _execute(subWorkflow, currentState, { handlers, registry, maxSteps: maxSteps - steps, signal, _nodeIdPrefix: prefixedNodeId }, emit);
      } else {
